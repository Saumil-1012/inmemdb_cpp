// tests.cpp
//
// Unit tests for the in‑memory database using Catch2.  These tests
// exercise the core functionality including table creation, insertion,
// selection, updates, deletes and error handling.  Both happy path
// scenarios and failure cases are covered.
...
TEST_CASE("create/insert/select", "[basic]") {
    ...
    REQUIRE(std::get<Str>(result.rows[0][0]) == "Ana");
    REQUIRE(std::get<Int>(result.rows[0][1]) == 19);
    REQUIRE(std::get<Str>(result.rows[1][0]) == "Ben");
    REQUIRE(std::get<Int>(result.rows[1][1]) == 25);
}

TEST_CASE("update/delete", "[mutation]") {
    Database db;
    ...
    REQUIRE(std::get<Str>(result.rows[0][0]) == "Bob");
    REQUIRE(std::get<Int>(result.rows[0][1]) == 40);
}

TEST_CASE("error cases", "[errors]") {
    Database db;
    // Duplicate table names should throw
    {
        ...
        REQUIRE_THROWS_AS(db.exec(*static_cast<CreateTable*>(stmt2.get())), std::exception);
    }
    // Unknown table on insert should throw
    {
        auto ti = tokenize("INSERT INTO no_table (x) VALUES (1)");
        TokenStream ts{ti};
        auto stmt = parse_statement(ts);
        REQUIRE_THROWS_AS(db.exec(*static_cast<Insert*>(stmt.get())), std::exception);
    }
    // Type mismatch on insert should throw
    {
        ...
        TokenStream ts2{ti};
        auto stmt2 = parse_statement(ts2);
        REQUIRE_THROWS_AS(db.exec(*static_cast<Insert*>(stmt2.get())), std::exception);
    }
}